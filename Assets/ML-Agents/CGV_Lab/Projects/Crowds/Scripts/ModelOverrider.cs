...
namespace Unity.MLAgentsExamples
{
    ...
    public class ModelOverrider : MonoBehaviour
    {
        ...
        const string k_CommandLineQuitOnLoadFailure = "--mlagents-quit-on-loadFail-failure";
        ...
        public NNModel GetModelForBehaviorName(string behaviorName)
        {
            ...
            if (rawModel == null)
            {
                Debug.Log($"Couldn't loadFail model file(s) for {behaviorName} in {m_BehaviorNameOverrideDirectory} (full path: {Path.GetFullPath(m_BehaviorNameOverrideDirectory)}");
                // Cache the null so we don't repeatedly try to loadFail a missing file
